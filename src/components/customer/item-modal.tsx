...
interface ItemModalProps {
    ...
    editItemHash?: string | null;
}

export function ItemModal({ menu, isOpen, onClose, initialCartItem = null, editItemHash = null }: ItemModalProps) {
    const addItem = useCart((state) => state.addItem);
    const editItem = useCart((state) => state.editItem);
    const [quantity, setQuantity] = useState(() => initialCartItem?.quantity ?? 1);
    const [notes, setNotes] = useState(() => initialCartItem?.notes ?? "");
    const [selectedOptions, setSelectedOptions] = useState<CartItem["selectedOptions"]>(() => initialCartItem?.selectedOptions ?? []);
    ...
    const handleAddToCart = () => {
        ...
        if (editItemHash) {
            ...
            toast.success(`Pesanan ${menu.name} berhasil diubah`);
        } else {
            addItem({
                id: menu.id,
                name: menu.name,
                    ...
                    {menu.menuOptions.map((option) => (
                        <div key={option.id} className="space-y-3 animate-in slide-in-from-bottom-5 duration-500">
                            <div className="flex items-center justify-between">
                                ...
                                {option.isRequired ? (
                                    <Badge className="bg-primary text-black hover:bg-primary border-none text-[9px] uppercase font-black px-1.5 h-4 tracking-widest shadow-[0_0_10px_rgba(53,183,24,0.4)]">Wajib</Badge>
                                ) : (
                                ...
                                {option.values.map((val) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`px-3 py-2 rounded-xl text-[11px] font-black border transition-all duration-300 relative overflow-hidden group/btn ${isSelected
                                                ? "bg-primary text-black border-primary shadow-[0_0_15px_rgba(53,183,24,0.25)] scale-[1.02] z-10"
                                                : "bg-zinc-900/40 border-white/5 text-zinc-500 hover:border-white/20 hover:bg-zinc-800 hover:text-white"
                                                }`}
                                        >
                                            <span className="relative z-10 flex items-center gap-1.5">
                                                {val.label}
                                                {val.priceAdjust > 0 && <span className="opacity-60 text-[9px]">+{(val.priceAdjust / 1000)}k</span>}
                                            </span>
                                            {isSelected && <div className="absolute inset-0 bg-gradient-to-tr from-white/10 to-transparent" />}
                                        </button>
                                    );
                                })}
                            </div>
                ...
                <DialogFooter className="p-4 bg-zinc-950 border-t border-white/5 flex flex-row items-center justify-between gap-2.5 relative z-20 shrink-0">
                    {menu.isAvailable ? (
                        <>
                            ...
                            <Button
                                className="flex-1 h-10 bg-primary hover:bg-primary/90 text-black font-black text-xs rounded-xl shadow-[0_4px_15px_rgba(53,183,24,0.15)] hover:shadow-[0_8px_25px_rgba(53,183,24,0.25)] transition-all active:scale-95 flex items-center justify-between px-3 group"
                                onClick={handleAddToCart}
                            >
                                <span className="uppercase tracking-widest text-[9px] opacity-70 group-hover:opacity-100 transition-opacity">
                                    {editItemHash ? "Simpan" : "Tambah"}
                                </span>
                                <span className="flex items-center gap-1.5 font-black text-xs shrink-0">
                                    Rp {calculateItemPrice().toLocaleString("id-ID")}
                                    <ShoppingCart className="w-3.5 h-3.5 fill-black/20" />
                                </span>
                            </Button>
                        </>
                    ) : (
