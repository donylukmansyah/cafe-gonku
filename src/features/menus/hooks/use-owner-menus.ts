...
import { showConfirm, showSuccess, showError } from "@/shared/client/sweetalert";
...
export function useOwnerMenus() {
    ...
    const deleteMenu = useCallback(async (id: string) => {
        const result = await showConfirm(
            "Hapus Menu?",
            "Menu akan dihapus dari daftar owner dan customer. Riwayat order tetap aman.",
            "Ya, Hapus",
            "warning"
        );
        ...
        if (result.isConfirmed) {
            ...
            try {
                ...
                await showSuccess("Berhasil", "Menu berhasil dihapus");
